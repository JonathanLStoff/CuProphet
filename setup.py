...
HEADERFILELOC = '\'' + os.getcwd()+"/hppfiles/cuda_func.hpp\'"
...
def build_cmdstan_model(target_dir):
    ...
    if MODEL_GPU and MODEL_MULTI_CORES:
        cpp_options = {
            ...
            'DSTAN_THREADS' : True,
            'USER_HEADER' : HEADERFILELOC,
            'TBB_CXX_TYPE':'gcc',
            'STAN_NO_RANGE_CHECKS': True,
            'STANCFLAGS' : '--O0 --allow-undefined --include-paths '+path 
            
        }
        stanc_options = {'allow-undefined' : True, 'include-paths': [path], 'fmad=false': True}
    ...
    if MODEL_GPU or MODEL_MULTI_CORES:
        sm = cmdstanpy.CmdStanModel(stan_file=os.path.join(MODEL_DIR, model_name), cpp_options=cpp_options, stanc_options=stanc_options)
    else:
